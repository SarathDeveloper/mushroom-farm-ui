        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
          {orders.length === 0 ? (
            <FadeIn className="flex flex-col items-center text-center py-16">
              ...
              <h2 className="text-2xl sm:text-3xl font-bold font-heading text-foreground mb-2">No orders yet</h2>
                  ...
                  <FadeIn className="p-4 sm:p-5 rounded-2xl border border-border bg-card hover:border-primary/30 transition-colors">
                    <div className="flex items-start sm:items-center justify-between gap-3">
                      <div className="min-w-0">
                        <p className="font-semibold text-foreground text-sm sm:text-base">#{order.id.slice(0, 8).toUpperCase()}</p>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <Badge variant={statusVariant[order.status] ?? "secondary"} className="text-sm">{order.status}</Badge>
